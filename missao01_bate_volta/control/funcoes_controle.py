...
from utils import centralization
from data_transfer import _file_lock, TARGET_JSON_PATH
import time
import json
...
TARGET_JSON_PATH = "alvo_data.json"            # Arquivo de Visão
...
def varredura_arena(Uno, passo_frente=0.5, velocidade_lateral=0.2,velocidade_frente=0.5, intervalo_checagem = 0.2):
    
    arena_largura = 10.0
    arena_profundidade = 10.0
    print(f"Iniciando busca do alvo (Checagem a cada {intervalo_checagem}s).")
    ...
    num_chegagens_lateral = int(tempo_varredura_lateral / intervalo_checagem)
    num_chegagens_frente = int(tempo_passo_frente / intervalo_checagem)

    print(f"Passo 0: Movendo {arena_largura}m para a esquerda...")
    for _ in range(num_chegagens_lateral):
        movimentacao_velocidade(Uno, 0, -velocidade_lateral, 0, intervalo_checagem)
        result_arq_alvo = ler_arquivo_alvo()
        if result_arq_alvo["found"]:
            print("Alvo encontrado! Parando varredura.")
            #print(f"{ler}")
            print(f"arquivo lido:{result_arq_alvo}")
            movimentacao_velocidade(Uno, 0, 0, 0, 3)
    ...
    for i in range(num_passos):
        ...
        for _ in range(num_chegagens_frente):
            movimentacao_velocidade(Uno, velocidade_frente, 0, 0, intervalo_checagem)
            result_arq_alvo = ler_arquivo_alvo()
            if result_arq_alvo["found"]:
                print("Alvo encontrado! Parando varredura.")
                print(f"arquivo lido:{result_arq_alvo}")
        ...
        for _ in range(num_chegagens_lateral):
            movimentacao_velocidade(Uno, 0, vy_atual, 0, intervalo_checagem)
...
def centralizacao(Uno, tentativas_max=1000, px_to_m=PIXEL_TO_METER, threshold_m=CENTER_THRESHOLD_M, step_speed=0.4, step_time=1.0):
    ...
    while tentativas < tentativas_max :
        ...
        #s_ , dx_m, dy_m = calculate_control_data(frame,"circulo", settings.FOCAL_LENGTH_PIXELS)
...
def posicao_drone(Uno):
    pos_x = Uno.location.local_frame.north
    pos_y = Uno.location.local_frame.east
